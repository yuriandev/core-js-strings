...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((acc, code) => {
    return acc + code.charCodeAt(0);
  }, 0);
}
...
function isPalindrome(str) {
  const justStr = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  return justStr === justStr.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  arrayWords.sort((a, b) => b.length - a.length);
  return arrayWords[0];
}
...
function reverseWords(str) {
  ...
  const reverseArrayWords = arrayWords.map((word) =>
    word.split('').reverse().join('')
  );
  return reverseArrayWords.join(' ');
}
...
function encodeToRot13(str) {
  ...
  const str2 = 'NOPQRSTUVWXYZABCDEFGHIJKLM'.toLowerCase();
  const newStr = str.replace(/[a-z]/gi, (v) => {
    const upLetter = v === v.toUpperCase();
    return upLetter
      ? str2[str1.indexOf(v.toLowerCase())].toUpperCase()
      : str2[str1.indexOf(v.toLowerCase())].toLowerCase();
  });
  return newStr;
}
